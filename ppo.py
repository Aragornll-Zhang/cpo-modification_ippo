...
class PPO:
	...
	def __init__(self, policy_class, env, **hyperparameters):
		...
		self.actor = policy_class(self.obs_dim, self.act_dim , probs = True ) # 离散空间输出概率                                                   # ALG STEP 1
	...
	def learn(self, total_timesteps):
		...
		while t_so_far < total_timesteps:                                                                       # ALG STEP 2
			...
			# ...
			with open('./results_ipo.jsonl', 'a+', encoding='utf-8') as jsonl_file:
			...
			if i_so_far % self.save_freq == 0 or (win_rate > 0.95 and constrain_hole < self.constrain_hole_prob):
				torch.save(self.actor.state_dict(), './model_results/orign_ppo_actor_iter{}.pth'.format(i_so_far) )
				torch.save(self.critic.state_dict(), './model_results/origin_ppo_critic_iter{}.pth'.format(i_so_far) )
				print('save...')
				with open('./save_torch_info_ppo.txt', 'a+', encoding='utf-8') as f:
		...
		while t < self.timesteps_per_batch:
			...
			if ep_constrain_dict['hole'] == 1:
				if 'hole_last_step' in batch_constrain_dict:
					...
					batch_constrain_dict['hole_last_step'] = [
						(batch_obs[-1], batch_acts[-1])]  # 记录上一个 (state , action) , 算概率，让该概率越小越好
